 ...
 *
 */
#define OS_TASK_NOTIFY(task, value, action) xTaskNotify((task), (value), (eNotifyAction)(action))
...
#define OS_TASK_NOTIFY_FROM_ISR(task, value, action) \
        ({ \
                ...
                ret = xTaskNotifyFromISR(task, value, (eNotifyAction)action, &need_switch); \
...
#define portCONVERT_MS_2_TICKS(x)  ( (x) / 10 )
